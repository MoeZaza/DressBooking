  ...
  return defineConfig({
    plugins: [
      ...
      createHtmlPlugin({
        inject: {
          data: {
            WEBSITE_NAME: process.env.VITE_BC_WEBSITE_NAME || 'BookDress',
          },
        },
      }),
    ],
    ...
    server: {
      host: '0.0.0.0',
      port: Number.parseInt(process.env.VITE_PORT || '3000', 10),
      watch: {
        usePolling: true,
        interval: 500,
      },
      hmr: {
        ...
        port: Number.parseInt(process.env.VITE_HMR_PORT || '3000', 10),
        clientPort: Number.parseInt(process.env.VITE_HMR_CLIENT_PORT || '3000', 10),
      },
      proxy: {
        '/api': {
          target: process.env.VITE_BC_API_HOST || 'http://localhost:4002',
          changeOrigin: true,
          secure: false,
        },
        '/cdn': {
