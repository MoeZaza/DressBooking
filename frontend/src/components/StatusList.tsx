...
const StatusList = ({
  ...
  const [value, setValue] = useState(statusListValue || '')

  useEffect(() => {
    if (statusListValue !== undefined && statusListValue !== value) {
      ...
      {disabled ? (
        ...
        <>
          ...
          <Select
            ...
            renderValue={(_value) => {
              if (!_value) {
                return <em style={{ color: '#999' }}>{label || 'Select status'}</em>
              }
              return <span className={`bs-s-sv bs-s-${_value}`}>{helper.getBookingStatus(_value as bookcarsTypes.BookingStatus)}</span>
            }}
          >
            {!required && (
              <MenuItem value="">
