...
const CreateUser = () => {
  ...
  const validateBirthDate = (_date?: Date) => {
  ...
  const customer = type === bookcarsTypes.RecordType.User
        ...
        <div className="create-user">
          ...
          <Paper className="user-form user-form-wrapper" elevation={10} style={visible ? {} : { display: 'none' }}>
            <form onSubmit={handleSubmit}>
              ...
              {customer && (
                <>
                  <FormControl fullWidth margin="dense">
                    ...
                    />
                    <FormHelperText error={!birthDateValid}>{''}</FormHelperText>
                  </FormControl>
              ...
              <FormControl fullWidth margin="dense">
                <InputLabel className={customer ? 'required' : ''}>{commonStrings.PHONE}</InputLabel>
                <Input id="phone" type="text" onBlur={handlePhoneBlur} onChange={handlePhoneChange} error={!phoneValid} required={!!customer} autoComplete="off" />
              ...
              <div className="info" style={{ marginTop: '16px', marginBottom: '16px' }}>
                <InfoIcon />
                <span>The user will receive an activation email to set their password and complete account setup.</span>
              </div>
