...
const LocationSelectList = ({
  ...
  useEffect(() => {
    // Refetch data when supplier changes
    if (init) {
      setRows([])
      setPage(1)
      setKeyword('')
  ...
  const fetchData = async (_page: number, _keyword: string, onFetch?: bookcarsTypes.DataEvent<bookcarsTypes.Location>) => {
    try {
      if (fetch || _page === 1) {
        ...
        let _rows = _page === 1 ? _data.resultData : [...rows, ..._data.resultData]

        // Filter by supplier if specified
        if (supplier) {
          _rows = _rows.filter((location: any) => 
            location.supplier && location.supplier._id === supplier
          )
        }
