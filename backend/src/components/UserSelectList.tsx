...
const UserSelectList = ({
  ...
  const [customers, setCustomers] = useState<bookcarsTypes.Option[]>([])
  ...
  const getCustomers = (users: bookcarsTypes.User[]): bookcarsTypes.Option[] =>
    users.map((user) => {
  ...
  const fetchData = async (_page: number, _keyword: string, onFetch?: bookcarsTypes.DataEvent<bookcarsTypes.User>) => {
    try {
      ...
      const data = await UserService.getCustomers(_keyword, _page, env.PAGE_SIZE)
      ...
      const customerOptionsList = getCustomers(_data.resultData)
      const _customers = _page === 1 ? customerOptionsList : [...customers, ...customerOptionsList]

      setCustomers(_customers)
      setFetch(customerOptionsList.length > 0)
