...
import { strings as csStrings } from '@/lang/dresses'
...
const BookingList = ({
  ...
  // Use context language if available, fallback to prop language
  const currentLanguage = contextLanguage || language || 'en'
    ...
    const _columns: GridColDef<bookcarsTypes.Booking>[] = [
      {
        ...
        flex: 1,
        renderCell: ({ row, value }: GridRenderCellParams<bookcarsTypes.Booking, string>) => <Link href={`/user?u=${(row.customer as bookcarsTypes.User)._id}`}>{value}</Link>,
      ...
      },
      {
        ...
        valueGetter: (value: number) => bookcarsHelper.formatPrice(value, commonStrings.CURRENCY, currentLanguage),
      },
      {
        field: 'status',
      ...
      {
        field: 'paymentStatus',
        headerName: strings.PAYMENT_STATUS,
        flex: 1,
        renderCell: ({ value }: GridRenderCellParams<bookcarsTypes.Booking, string>) => {
          ...
          const getArabicLabel = (status: string) => {
            switch (status) {
              case 'fully-paid': return strings.FULLY_PAID
              case 'partially-paid': return strings.PARTIALLY_PAID
              case 'pending': return strings.PENDING
              case 'refunded': return strings.REFUNDED
              default: return strings.PENDING
            }
          }
          return <Chip label={getArabicLabel(value || 'pending')} color={getColor(value || 'pending') as any} size="small" />
        },
      ...
      {
        field: 'paidAmount',
        headerName: strings.PAID_AMOUNT,
        flex: 1,
        renderCell: ({ row }: GridRenderCellParams<bookcarsTypes.Booking>) => (
          <span>
            {bookcarsHelper.formatPrice(row.paidAmount || 0, commonStrings.CURRENCY, currentLanguage)} / {bookcarsHelper.formatPrice(row.price || 0, commonStrings.CURRENCY, currentLanguage)}
            {row.remainingAmount && row.remainingAmount > 0 && (
              <div style={{ fontSize: '0.8em', color: 'orange' }}>
                {strings.REMAINING}: {bookcarsHelper.formatPrice(row.remainingAmount, commonStrings.CURRENCY, currentLanguage)}
              </div>
            )}
          </span>
        ),
        valueGetter: (value: number, row: bookcarsTypes.Booking) => `${bookcarsHelper.formatPrice(value || 0, commonStrings.CURRENCY, currentLanguage)} / ${bookcarsHelper.formatPrice(row.price || 0, commonStrings.CURRENCY, currentLanguage)}`,
      },
      {
        field: 'action',
        headerName: '',
    ...
    if (!hideDressColumn) {
      _columns.unshift({
        ...
        renderCell: ({ row, value }: GridRenderCellParams<bookcarsTypes.Booking, string>) => (
          <div>
            <Link href={`/dress?dr=${(row.dress as any)?._id}`}>{value}</Link>
            <div style={{ fontSize: '0.8em', color: 'gray' }}>
              {strings.DRESS_CODE}: {(row.dress as any)?.dressCode || 'N/A'}
            </div>
          </div>
        ),
  ...
  const handleEditSave = (updatedBooking: bookcarsTypes.Booking) => {
    // Update the booking in the rows
    const updatedRows = rows.map(row =>
      row._id === updatedBooking._id ? updatedBooking : row
    )
    setRows(updatedRows)
  ...
  const _fr = currentLanguage === 'fr'
  const _ar = currentLanguage === 'ar'
  const _locale = _fr ? dfnsFR : _ar ? dfnsAR : dfnsENUS
            ...
            {rows.map((booking, index) => {
              ...
              return (
                <div key={booking._id} className="booking-details">
                  ...
                  <div className="booking-detail" style={{ height: bookingDetailHeight }}>
                    <span className="booking-detail-title">{strings.CUSTOMER}</span>
                    <div className="booking-detail-value">
                      <Link href={`user/?u=${(booking.customer as bookcarsTypes.User)._id}`}>{(booking.customer as bookcarsTypes.User).fullName}</Link>
                    </div>
                  </div>
                  <div className="booking-detail" style={{ height: bookingDetailHeight }}>
                  ...
                  <div className="booking-detail" style={{ height: bookingDetailHeight }}>
                    ...
                    <div className="booking-detail-value">{(booking.location as bookcarsTypes.Location).name}</div>
                  </div>
                  ...
                  <div className="booking-detail" style={{ height: bookingDetailHeight }}>
                    ...
                    <div className="booking-detail-value booking-price">{bookcarsHelper.formatPrice(booking.price as number, commonStrings.CURRENCY, currentLanguage)}</div>
                  </div>
