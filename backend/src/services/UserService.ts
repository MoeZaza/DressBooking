 ...
 */
export const create = (data: bookcarsTypes.CreateUserPayload): Promise<bookcarsTypes.User> =>
  axiosInstance
    .post(
      '/api/create-user',
      data,
 ...
 */
export const getCustomers = (keyword: string, page: number, size: number): Promise<bookcarsTypes.Result<bookcarsTypes.User>> =>
  axiosInstance
    .post(
...
export const authHeader = async (): Promise<{ [key: string]: string }> => {
  // For backend, we use cookies for authentication
  // Return empty headers as authentication is handled by cookies
  return {}
}
