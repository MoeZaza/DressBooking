 ...
 */
export const getSuppliers = (keyword: string): Promise<bookcarsTypes.Result<bookcarsTypes.User>> =>
  axiosInstance
    .post(
      `/api/backend-suppliers?s=${encodeURIComponent(keyword)}`,
      { suppliers: [] }, // Send GetDressesPayload format
      { withCredentials: true }
    )
    .then((res) => {
      // Backend suppliers API returns suppliers directly, not paginated
      // Convert to Result format for compatibility
      const suppliers = Array.isArray(res.data) ? res.data : []
      return [{
        resultData: suppliers,
        pageInfo: { totalRecords: suppliers.length }
      }]
    })
