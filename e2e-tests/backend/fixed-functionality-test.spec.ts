...
/**
 * Fixed Backend Functionality Test
 * Tests all backend pages without localStorage issues
 */

test.describe('Fixed Backend Functionality Tests', () => {
  let page: Page
  ...
  async function loginAsAdmin(page: Page): Promise<boolean> {
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        console.log(`🔐 Logging in as admin (attempt ${attempt}/3)...`)

        // Add delay between attempts to avoid security blocking
        if (attempt > 1) {
        ...
        // Wait for redirect to dashboard with more flexible URL matching
  ...
  test('should test Suppliers Management functionality', async () => {
    ...
    const addButton = await page.locator('button:has-text("Add"), button:has-text("Create"), .new-supplier, button:has-text("New")').count()
    ...
    // Check for supplier list or table
    const supplierList = await page.locator('.MuiDataGrid-root, .MuiTable-root, .supplier-list, .data-grid').count()
    console.log(`📋 Supplier lists found: ${supplierList}`)
  ...
  test('should test Locations Management functionality', async () => {
    ...
    const locationElements = await page.locator('.location, .MuiCard-root, .location-item').count()
  ...
  test('should test Dresses Management functionality', async () => {
    ...
    const dressElements = await page.locator('.dress, .MuiCard-root, .new-dress, .dress-item').count()
  ...
  test('should test Users Management functionality', async () => {
    ...
    const userElements = await page.locator('.user, .MuiCard-root, .user-item').count()
