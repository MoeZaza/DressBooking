  ...
  try {
    ...
    if (country) {
      ...
      if (country.values && Array.isArray(country.values)) {
        const countryValue = (country.values as env.LocationValue[]).filter((value) => value.language === req.params.language)[0]
        name = countryValue ? countryValue.value : 'Unknown Country'
      }
...
export const getCountries = async (req: Request, res: Response) => {
  try {
    ...
    const keyword = searchQuery ? escapeStringRegexp(searchQuery.trim()) : ''
    const options = 'i'
    ...
    // Only add regex match if keyword is not empty
    if (keyword) {
      matchConditions.push({ $expr: { $regexMatch: { input: '$value', regex: keyword, options } } })
    }

    const countries = await Country.aggregate(
      [
        // First filter out documents without proper values array
        {
          $match: {
            values: { $exists: true, $type: 'array', $ne: [] }
          }
        },
        {
          $lookup: {
