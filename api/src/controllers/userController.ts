...
const _signup = async (req: Request, res: Response, userType: bookcarsTypes.UserType) => {
  ...
  try {
    ...
    // Return the created user data for frontend use
    const userData = {
      _id: user._id,
...
export const create = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Don't set password during creation - user will set it during activation
    const { contracts } = body
    body.contracts = undefined
    body.password = undefined // Remove password from body

    const user = new User({
      ...body,
      active: false // User needs to activate account first
    })
    await user.save()
    ...
    // Note: For development, auto-activate users instead of sending email
    if (process.env.NODE_ENV === 'development' || !env.SMTP_HOST || env.SMTP_HOST === 'smtp.sendgrid.net') {
      // Auto-activate user for development
      user.active = true
      user.verified = true
      ...
      const salt = await bcrypt.genSalt(10)
      const defaultPassword = user.type === bookcarsTypes.UserType.Admin ? 'admin123' :
                             user.type === bookcarsTypes.UserType.Supplier ? 'supplier123' : 'customer123'
      user.password = await bcrypt.hash(defaultPassword, salt)

      await user.save()

      logger.info(`[user.create] User ${user.email} created and auto-activated for development`)
      res.json({ _id: user._id, email: user.email, fullName: user.fullName, type: user.type })
    } else {
      // Production: generate token and send activation email
      ...
      const mailOptions: nodemailer.SendMailOptions = {
        ...
        html:
          `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
            <h2 style="color: #333; text-align: center;">Welcome to BookDress</h2>
            <p style="font-size: 16px; color: #555;">
              ${i18n.t('HELLO')} ${user.fullName},<br><br>
              An account has been created for you by an administrator. To complete your account setup and set your password, please click the link below:<br><br>
              <a href="${activationLink}" target="_blank" style="display: inline-block; padding: 12px 24px; background-color: #007bff; color: white; text-decoration: none; border-radius: 4px;">Activate Account & Set Password</a><br><br>
      ...
      logger.info(`[user.create] User ${user.email} created and activation email sent`)
      res.sendStatus(200)
    }
  } catch (err) {
...
export const validateEmail = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Fix the $in query issue by using $or instead
...
export const update = async (req: Request, res: Response) => {
  try {
    ...
    if (locations) {
      user.locations = locations.map(loc => new mongoose.Types.ObjectId(loc))
    }
    user.bio = bio
...
export const getUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const user = await User.findById(id, {
      ...
      priceChangeRate: 1,
      supplierDressLimit: 1,
      notifyAdminOnNewDress: 1,
    }).lean()
    ...
    if (user.avatar) {
      user.avatar = helper.getCdnUrl(user.avatar, 'users')
    }
...
export const getUsers = async (req: Request, res: Response) => {
  try {
    ...
    const users = await User.find($match)
      .select('supplier email phone fullName verified language enableEmailNotifications avatar bio location type blacklisted birthDate customerId')
