 ...
 * @param {?number} [accessoryTotal] - Pre-calculated accessory total from AccessorySettings
 * @returns {number}
 */
export const calculateTotalPriceWithAccessories = (
  dress: bookcarsTypes.Dress,
  priceChangeRate: number,
  options?: bookcarsTypes.DressOptions,
  accessoryTotal?: number
) => {
  // Use fixed dress price (discounted price if available, otherwise regular price)
  let totalPrice = (dress.discountedPrice || dress.price) || 0

  // Add extra options with fixed costs
  if (options) {
    if (options.cancellation && dress.cancellation && dress.cancellation > 0) {
      totalPrice += dress.cancellation
    }
    if (options.amendments && dress.amendments && dress.amendments > 0) {
      totalPrice += dress.amendments
    }
    if (options.accessories && dress.accessories && dress.accessories.length > 0) {
      // Use dynamic accessory pricing if provided, otherwise fallback to fixed fee
      totalPrice += accessoryTotal !== undefined ? accessoryTotal : 50
    }
  }
...
/**
 * Calculate total price for dress rental (legacy function for backward compatibility).
 *
 * @param {bookcarsTypes.Dress} dress
 * @param {number} priceChangeRate
 * @param {?bookcarsTypes.DressOptions} [options]
 * @returns {number}
 */
export const calculateTotalPrice = (dress: bookcarsTypes.Dress, priceChangeRate: number, options?: bookcarsTypes.DressOptions) => {
  return calculateTotalPriceWithAccessories(dress, priceChangeRate, options)
}
